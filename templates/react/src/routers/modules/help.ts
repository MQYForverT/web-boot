 ...
 * @returns routeListFlat: 处理后的平板路由列表
 * @returns keepAliveList: 需要缓存的路由列表
 */
export const getPermissionData = (
	localRoutes: Menu.MenuOptions<LazyExoticComponent<any>>[],
	result: Menu.permissionMenu[],
	tree: boolean = false,
) => {
	if (tree) {
	...
	const keepAliveList: Menu.MenuOptions<LazyExoticComponent<any>>[] = []
	const routeListFlat: Menu.MenuOptions<LazyExoticComponent<any>>[] = []
	const sortMap = new Map(result.map((item) => [item.permission, item.sort || 1]))

	// 递归处理路由及其子路由
	const processRoutes = (routes: Menu.MenuOptions<LazyExoticComponent<any>>[]) => {
		return routes
			.filter((item) => {
				// 有权限
