...
function fe(e, t, n = '/') {
	...
	return (
		g < i && g >= 0 && (i = -1),
		i > -1 && ((r = t.slice(0, i)), (a = t.slice(i + 1, g > -1 ? g : t.length)), (o = e(a))),
		g > -1 && ((r = r || t.slice(0, g)), (d = t.slice(g, t.length))),
		(r = Ot(r ?? t, n)),
		{ fullPath: r + (a && '?') + a + d, path: r, query: o, hash: X(d) }
	)
}
function bt(e, t) {
...
function Qe(e, t) {
	...
	for (const n in e) if (!_t(e[n], t[n])) return !1
	return !0
}
function _t(e, t) {
...
let Bt = () => location.protocol + '//' + location.host
function Fe(e, t) {
...
function Ht(e, t, n, r) {
	...
	return (
		...
		{ pauseListeners: i, listen: f, destroy: c }
	)
}
function be(e, t, n, r = !1, o = !1) {
...
function jt(e) {
	const { history: t, location: n } = window,
		...
		o = { value: t.state }
	o.value ||
		a(r.value, { back: null, current: r.value, forward: null, position: t.length - 1, replaced: !0, scroll: null }, !0)
	function a(i, f, h) {
		const c = e.indexOf('#'),
			l = c > -1 ? (n.host && document.querySelector('base') ? e : e.slice(c)) + i : Bt() + e + i
		try {
	...
	function d(i, f) {
		const h = P({}, t.state, be(o.value.back, i, o.value.forward, !0), f, { position: o.value.position })
		;(a(i, h, !0), (r.value = i))
	}
	function g(i, f) {
...
function K(e, t) {
	return P(new Error(), { type: e, [Xe]: !0 }, t)
}
function L(e, t) {
	return e instanceof Error && Xe in e && (t == null || !!(e.type & t))
}
const _e = '[^/]+?',
	qt = { sensitive: !1, strict: !1, start: !0, end: !0 },
	Gt = /[.+*?^${}()[\]/\\]/g
function Kt(e, t) {
	const n = P({}, qt, t),
		r = []
		...
		for (const l of e) {
			...
			for (const u of l)
				if (u.type === 0) h += u.value
					...
					const { value: w, repeatable: S, optional: b } = u,
						A = w in f ? f[w] : ''
					if (T(A) && !S)
						throw new Error(`Provided param "${w}" is an array but it is not repeatable (* or + modifiers)`)
					const E = T(A) ? A.join('/') : A
					if (!E)
						if (b) l.length < 2 && (h.endsWith('/') ? (h = h.slice(0, -1)) : (c = !0))
...
function Yt(e, t) {
	const n = [],
		r = new Map()
	t = Te({ strict: !1, end: !0, sensitive: !1 }, t)
	function o(c) {
		return r.get(c)
	}
	function a(c, l, u) {
		...
		if ('alias' in c) {
			...
			for (const j of I)
				A.push(Me(P({}, S, { components: u ? u.record.components : S.components, path: j, aliasOf: u ? u.record : S })))
		}
		let E, k
		for (const I of A) {
			const { path: j } = I
			if (l && j[0] !== '/') {
				const $ = l.record.path,
					M = $[$.length - 1] === '/' ? '' : '/'
				I.path = l.record.path + (j && M + j)
			}
			if (
				((E = Ft(I, l, b)),
				u ? u.alias.push(E) : ((k = k || E), k !== E && k.alias.push(E), w && c.name && !Ie(E) && d(c.name)),
				Je(E) && i(E),
				S.children)
			) {
				const $ = S.children
			...
			;((b = u.record.name),
				(w = P(
					xe(
						...
						u.keys
							.filter((k) => !k.optional)
							.concat(u.parent ? u.parent.keys.filter((k) => k.optional) : [])
							.map((k) => k.name),
					),
					c.params &&
						xe(
...
function Ne(e) {
	...
	for (let n in e) {
		...
		;(T(r) ? r.map((a) => a && de(a)) : [r && de(r)]).forEach((a) => {
...
function $e(e) {
	const t = q(ne),
		...
		a = N(() => o.value > -1 && ln(n.params, r.value.params)),
	...
	return { route: r, href: N(() => r.value.href), isActive: a, isExactActive: d, navigate: g }
}
function on(e) {
...
function ln(e, t) {
	for (const n in t) {
		...
		if (typeof r == 'string') {
			if (r !== o) return !1
		} else if (!T(o) || o.length !== r.length || r.some((a, d) => a !== o[d])) return !1
	}
	return !0
}
function Be(e) {
					...
					([f, h, c], [l, u, w]) => {
						;(h &&
							...
							u &&
								...
								f &&
								f === l &&
								(h.leaveGuards.size || (h.leaveGuards = u.leaveGuards),
								h.updateGuards.size || (h.updateGuards = u.updateGuards))),
							f && h && (!u || !G(h, u) || !l) && (h.enterCallbacks[c] || []).forEach((S) => S(f)))
					},
					{ flush: 'post' },
				),
				() => {
					const f = o.value,
						...
						c = g.value,
						l = c && c.components[h]
					if (!l) return je(n.default, { Component: l, route: f })
					const u = c.props[h],
					...
					return je(n.default, { Component: b, route: f }) || b
				}
			)
		},
	})
...
function mn(e) {
	...
	function u(s, m) {
		let p, v
		return (Ye(s) ? ((p = t.getRecordMatcher(s)), (v = m)) : (v = s), t.addRoute(v, p))
	}
	function w(s) {
	...
	function A(s, m) {
		...
		const _ = bt(r, P({}, s, { hash: Et(C), path: v.path })),
			R = o.createHref(_)
		return P({ fullPath: _, hash: C, query: r === Ne ? nn(s.query) : s.query || {} }, v, {
			redirectedFrom: void 0,
			href: R,
		})
	}
	...
	function $(s) {
		...
		if (m && m.redirect) {
			...
			return (
				typeof v == 'string' && ((v = v.includes('?') || v.includes('#') ? (v = E(v)) : { path: v }), (v.params = {})),
				P({ query: s.query, hash: s.hash, params: v.path != null ? {} : s.params }, v)
			)
		}
	}
	function M(s, m) {
		const p = (f = A(s)),
			...
			R = s.replace === !0,
			y = $(p)
		if (y) return M(P(E(y), { state: typeof y == 'object' ? P({}, C, y.state) : C, force: _, replace: R }), m || p)
		const O = p
		O.redirectedFrom = m
		let D
		return (
			!_ && At(r, v, p) && ((D = K(16, { to: O, from: v })), we(v, v, !0, !1)),
			(D ? Promise.resolve(D) : ve(O, v))
				.catch((x) => (L(x) ? (L(x, 2) ? x : ie(x)) : se(x, O, v)))
				.then((x) => {
					if (x) {
						if (L(x, 2))
							return M(
								P({ replace: R }, E(x.to), { state: typeof x.to == 'object' ? P({}, C, x.to.state) : C, force: _ }),
								m || O,
							)
					} else x = Re(O, v, !0, R, C)
					return (ye(O, v, x), x)
	...
	function ve(s, m) {
		...
		return (
			...
			z(p)
				.then(() => {
					p = []
					for (const y of a.list()) p.push(H(y, s, m))
					return (p.push(R), z(p))
				})
				.then(() => {
				...
				.then(() => {
					...
					for (const y of _)
						if (y.beforeEnter)
							if (T(y.beforeEnter)) for (const O of y.beforeEnter) p.push(H(O, s, m))
							else p.push(H(y.beforeEnter, s, m))
					return (p.push(R), z(p))
				})
				.then(
					() => (
						s.matched.forEach((y) => (y.enterCallbacks = {})),
						(p = he(_, 'beforeRouteEnter', s, m, re)),
						p.push(R),
						z(p)
					),
				)
				.then(() => {
					p = []
					for (const y of d.list()) p.push(H(y, s, m))
					return (p.push(R), z(p))
				})
	...
	function Re(s, m, p, v, C) {
		...
		const R = m === B,
			y = V ? history.state : {}
		;(p && (v || R ? o.replace(s.fullPath, P({ scroll: R && y && y.scroll }, C)) : o.push(s.fullPath, C)),
			(i.value = s),
			we(s, m, p, R),
			ie())
	}
		...
		U ||
			(U = o.listen((s, m, p) => {
				...
				const v = A(s),
					C = $(v)
				if (C) {
					M(P(C, { replace: !0, force: !0 }), v).catch(F)
					return
				}
				f = v
				const _ = i.value
				;(V && Lt(Ce(_.fullPath, p.delta), te()),
					ve(v, _)
						.catch((R) =>
							L(R, 12)
								? R
								: L(R, 2)
									? (M(P(E(R.to), { force: !0 }), v)
											.then((y) => {
												L(y, 20) && !p.delta && p.type === Z.pop && o.go(-1, !1)
											})
											.catch(F),
										Promise.reject())
									: (p.delta && o.go(-p.delta, !1), se(R, v, _)),
						)
						.then((R) => {
							;((R = R || Re(v, _, !1)),
								R && (p.delta && !L(R, 8) ? o.go(-p.delta, !1) : p.type === Z.pop && L(R, 20) && o.go(-1, !1)),
								ye(v, _, R))
						})
						.catch(F))
			}))
	...
	const ee = new Set(),
		Pe = {
			...
			install(s) {
				const m = this
				;(s.component('RouterLink', cn),
					s.component('RouterView', fn),
					(s.config.globalProperties.$router = m),
					Object.defineProperty(s.config.globalProperties, '$route', { enumerable: !0, get: () => Q(i) }),
					V && !ae && i.value === B && ((ae = !0), I(o.location).catch((C) => {})))
				const p = {}
				for (const C in B) Object.defineProperty(p, C, { get: () => i.value[C], enumerable: !0 })
				;(s.provide(ne, m), s.provide(et, st(p)), s.provide(me, i))
				const v = s.unmount
				;(ee.add(s),
					(s.unmount = function () {
						;(ee.delete(s), ee.size < 1 && ((f = B), U && U(), (U = null), (i.value = B), (ae = !1), (J = !1)), v())
					}))
			},
		}
	function z(s) {
...
export { pn as a, mn as c, gn as u }
